...
3. 在插件配置填写 HTTP 或 HTTPS 服务根地址（不含 `/api/v1`）、上传凭据、活动 ID、稳定实例 ID 和允许群列表。
4. 初次没有配置时插件可以加载，但投稿与后台上传保持停用；配置保存后重载插件才会启用。凭据输入默认遮蔽。HTTP 地址也可使用公网域名或 IP；通过 HTTP 上传时，凭据和投稿内容以明文传输。
